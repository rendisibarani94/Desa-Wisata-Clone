...
@section('Judul_Halaman','Penginapan')
@section('Konten')
<div class="max-w-screen-xl mx-auto p-5 sm:p-10 md:p-16">
    ...
    Daftar Penginapan Yang Tersedia
</p> --}}
    <div class="flex items-center justify-center ">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 gap-6">
            @foreach ($data as $item)
            <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow-lg transition-transform hover:scale-105">
                {{-- {{ route('objek-wisata.detail', $item->id) }} --}}
                <a href="{{ route('user.fasilitas.penginapan.detail', $item->id) }}">
                    <img class="rounded-t-lg w-full h-72 object-cover" src="{{ asset('images/fasilitas/penginapan/'.$item->gambar1) }}" alt="" />
                </a>
                <div class="p-2">
                    <div class="flex justify-between">
                        {{-- {{ route('objek-wisata.detail', $item->id) }} --}}
                        <a href="{{ route('user.fasilitas.penginapan.detail', $item->id) }}">
                            <h5 class="text-black mr-2 text-xl font-semibold tracking-tight hover:text-blue-500">{{ $item->namaPenginapan }}</h5>
                        </a>
                    ...
                    <div class="text-right">
                        {{-- {{ route('objek-wisata.detail', $item->id) }} --}}
                        <a href="{{ route('user.fasilitas.penginapan.detail', $item->id) }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg focus:ring-4 focus:outline-none bg-orange-600 hover:bg-orange-700 focus:ring-orange-800">
                            Selengkapnya
